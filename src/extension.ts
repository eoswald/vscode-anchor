...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(
        vscode.languages.registerOnTypeFormattingEditProvider(
            { language: 'anchor', scheme: 'file' },
            new AnchorIndentProvider(),
            '\n'
        )
    );
}
...
class AnchorIndentProvider implements vscode.OnTypeFormattingEditProvider {
 ...
 */
function buildSkipRegions(text: string, from: number, to: number): SkipRegion[] {
...
function isInSkipRegion(regions: SkipRegion[], offset: number): boolean {
    let lo = 0;
